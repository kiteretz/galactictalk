...
">
	<div class="container relative z-10 grid gap-50 pt-20 ~pb-72/244">
		<div class="grid items-center gap-16 *:col-span-full *:row-span-full">
			<div class="leading-[1.04] tracking-[-0.03em] ~text-50/132 stroke-text *:inline-block">
				<span>The Power </span><span>of Language</span>
			</div>
			<div class="font-bold leading-normal ~text-10/28 ~px-1/4 ~pt-6/14 gradient-text">
				ワクワクする未来を話そう
			</div>
		</div>
		<div class="flex w-[13rem] flex-col ~gap-40/80 lg:w-auto lg:flex-row lg:items-start">
			<div class="grid max-w-[18.625rem] place-items-start gap-[1.6875rem]">
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>">
					<img class="~h-38/54 w-auto" src="<?php echo esc_url( get_theme_file_uri( '/assets/images/logo.svg' ) ); ?>" alt="<?php bloginfo( 'name' ); ?>">
				</a>
	...
	foreach ( array(
		array(
			'class'  => '-bottom-8 ~/lg:~left-[10.875rem]/[16.5rem] ~/lg:~w-260/360 lg:left-[63vw] lg:top-0 lg:w-[28.875rem]',
		...
		?>
		<img class="<?php cx( 'absolute rounded-t-full ~rounded-b-[2.5rem]/[6rem]', $image['class'] ); ?>" src="<?php echo esc_url( get_theme_file_uri( $image['src'] ) ); ?>" alt="" width="<?php echo esc_attr( $image['width'] ); ?>" height="<?php echo esc_attr( $image['height'] ); ?>">
	<?php endforeach; ?>
</footer>
